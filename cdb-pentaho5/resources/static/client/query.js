...
wd.cdb.QueryGroup = function(label, description) {
  ...
  $.getJSON("connector/exportCda?group=" + _label,function(){console.log("Saved to CDA")});
  };
...
wd.cdb.QueryManager = (function() {
    ...
    myself.loadGroup = function(group,callback) {
      $.getJSON("query/loadGroup",{group: group.getLabel()}, function(response){
        if(response) {
          var query, q, queryData;
          myself.addGroup(group);
          for (q in response.result) 
            if (response.result.hasOwnProperty(q)) {
    ...
    myself.loadGroupList = function(callback) {
      $.getJSON('query/listGroups',{},function(response){//query?method=listGroups
